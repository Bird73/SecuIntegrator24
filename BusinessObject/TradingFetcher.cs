...
public class TradingFetcher
{
    ...
    public void FetchListingTradingDataFromWeb(DateTime processDate)
    {
    ...
    public List<TradingData> FetchListingTradingDataFromJSON(DateTime processDate)
    {
        ...
        if (File.Exists(filePath))
        {
            ...
            catch (Exception ex)
            {
                ...
                EventLogManager.WriteEventLog(new EventLog
                {
                    Message = "Failed to fetch trading data from file: " + filePath,
                    Type = EventType.Error
                });
            }
        }
    ...
    private List<TradingData> ParseJsonData(string json)
    {
    ...
    public void SaveTradingToDatabase(List<TradingData> tradingData, DateTime tradingDate)
    {
        // Save the trading data to the database
        foreach (TradingData data in tradingData)
        {
            ...
            if (!string.IsNullOrEmpty(data.Code) && Regex.IsMatch(data.Code, @"^[1-9]\d{3}$"))
            {
                try
                {
                    ...
                    trading.MarketType = MarketType.Listing;
                    ...
                    trading.OpeningPrice = Convert.ToDecimal(data.OpeningPrice == "--" ? null : data.OpeningPrice.Replace(",", ""));
                    trading.HighestPrice = Convert.ToDecimal(data.HighestPrice == "--" ? null : data.HighestPrice.Replace(",", ""));
                    trading.LowestPrice = Convert.ToDecimal(data.LowestPrice == "--" ? null : data.LowestPrice.Replace(",", ""));
                    trading.ClosingPrice = Convert.ToDecimal(data.ClosingPrice == "--" ? null : data.ClosingPrice.Replace(",", ""));
                    ...
                    trading.Change = Convert.ToDecimal(data.Change.Replace(",", ""));
                    trading.FinalBidPrice = Convert.ToDecimal(data.FinalBidPrice == "--" ? null : data.FinalBidPrice.Replace(",", ""));
                    trading.FinalBidVolume = Convert.ToUInt64(data.FinalBidVolume.Replace(",", "") == "" ? null : data.FinalBidVolume.Replace(",", ""));
                    trading.FinalAskPrice = Convert.ToDecimal(data.FinalAskPrice == "--" ? null : data.FinalAskPrice.Replace(",", "")) ;
                    trading.FinalAskVolume = Convert.ToUInt64(data.FinalAskVolume.Replace(",", "") == "" ? null : data.FinalAskVolume.Replace(",", ""));
                    trading.PERatio = Convert.ToDecimal(data.PERatio.Replace(",", ""));
